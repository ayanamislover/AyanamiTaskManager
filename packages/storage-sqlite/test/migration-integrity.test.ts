    ...
    it(`从 v${legacyVersion} Registry 与项目库升级到当前 schema 并保留数据`, async () => {
      ...
      try {
        ...
        expect(
          upgraded.sqlite
            .prepare(
              "SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_work_items_task_list_keyset'",
            )
            .get(),
        ...
        expect(
          upgraded.sqlite
            .prepare(
              "SELECT name FROM sqlite_master WHERE type = 'index' AND name = 'idx_agent_sessions_list_keyset'",
            )
            .get(),
        ).toEqual({ name: "idx_agent_sessions_list_keyset" });
        expect(
