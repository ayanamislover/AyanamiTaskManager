...
const profileInstructions: Readonly<Record<AyanamiMcpProfile, string>> = Object.freeze({
  legacy:
    "MCP surface v5 · legacy 兼容入口为尚未重启的旧 Agent 会话保留冻结的 11 工具。请重启 Agent 客户端以重新加载已自动迁移的 core / memory / actions 三 Profile 配置；若仍只看到本入口，请在 ATM 设置中重新安装对应 Agent 集成。",
  core: "MCP surface v5 · core profile。开工调用一次 atm_begin 并直接使用返回的 brief；不要紧接 atm_brief。仅在上下文压缩、长时间离开或明确恢复 working set 时调用 atm_brief。task_list/task_get 按需，结束调用 atm_end。",
  memory:
    "MCP surface v5 · memory profile。Session 由 core profile 建立；本 profile 负责进度、长期记录、本机反馈、搜索与增量读取；共享知识按需读取。",
  actions:
...
export function createAyanamiMcpServer(
  ...
  const server = new Server(
    { name: "ayanami-task-manager", version: "1.0.27" },
