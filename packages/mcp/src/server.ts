...
const profileInstructions: Readonly<Record<AyanamiMcpProfile, string>> = Object.freeze({
  ...
  core: "MCP surface v3 · core profile。开工调用一次 atm_begin 并直接使用返回的 brief；不要紧接 atm_brief。仅在上下文压缩、长时间离开或明确恢复 working set 时调用 atm_brief。task_list/task_get 按需，结束调用 atm_end。",
  memory:
...
export function createAyanamiMcpServer(
  ...
  const server = new Server(
    { name: "ayanami-task-manager", version: "1.0.20" },
