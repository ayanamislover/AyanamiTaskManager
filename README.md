...
这一轮改进来自三个重度开发任务的实际反馈：**工具参数看得见，写入结果说得清，恢复上下文不靠猜。**

- **少一次无效重试。** 操作分支保留公共必填字段；字段掩码拼错会得到提示，字符预算不足会明确告诉调用方如何重试。
- **少一次补查状态。** 任务写回执携带本次状态与领取快照；阻塞可直接重新归类为等待，仍保留版本和租约校验。
- **交接与审核更明确。** 同线程暂停后可恢复已保存的交接；候选哈希不符时返回差异和查询入口，不降低审核约束。
- **升级不覆盖自定义技能。** 有基线才能判断是否安全更新；无基线或用户修改过的旧安装不会被静默替换。

桌面端同步改进设置布局、项目顺序和工程统计扫描；任务表更新时间完整显示，顶栏不再重复项目标题，任务区与管理卡片保持统一间距。1.1.3 将此前本地界面热修复统一纳入正式安装包。查看[本轮反馈与处理边界](./docs/agent-feedback-2026-09.md)。正式发布状态以 [GitHub Release](https://github.com/ayanamislover/AyanamiTaskManager/releases) 为准。
