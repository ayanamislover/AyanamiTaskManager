...
$vite = Start-Process -FilePath $node `
  -ArgumentList @("node_modules/vite/bin/vite.js", "--config", "apps/desktop/vite.config.ts", "--host", "127.0.0.1", "--port", "5174") `
  -WorkingDirectory $workspace -WindowStyle Hidden `
...
for ($attempt = 0; $attempt -lt 100; $attempt += 1) {
  try {
    ...
    $page = Invoke-WebRequest -Uri "http://127.0.0.1:5174" -TimeoutSec 1
