...
本清单适用于 Windows x64 的 1.0.9 发布。任何红项都必须修复或在 release notes 中明确列为非阻塞剩余项；不得把缺失产物写成“已完成”。
...
```powershell
pnpm exec tsx scripts/release-and-install.ts --version 1.0.9
```
...
```text
release/
├─ AyanamiTaskManager-Setup-1.0.9-win-x64.exe
├─ AyanamiTaskManager-1.0.9-win-x64-portable.zip
├─ SHA256SUMS.txt
├─ release.json
├─ sbom.spdx.json
└─ test-report/
